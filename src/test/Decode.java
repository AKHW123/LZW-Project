...
public class Decode {
	...
	/**
	 * Decompresses a file encoded with Tao-Zhang implementation of Lempel-Ziv-Welch
	 * encoding.
	 * 
	...
	public static String decode(String fileName) throws IOException {
		...
		try {
			...
			// tells the computer whether or not we are reading an actually valuable String
			// or if we're on a delimiter
			...
			int index = 256;
			// tells the computer whether or not we have started reading actual codes, or if
			// we are still reading dictionary entries, separated by an x
			boolean foundX = false;
			// start reading the file
			while ((characterValue = bufferedReader.read()) != -1) {
				// if we find the x, then we know to start reading in actual codes, so we
				// restart the while loop to ignore the x
				if (String.valueOf((char) characterValue).equals("x")) {
					foundX = true;
					continue;
				}
				...
				if (foundX == false) {
					...
					thisCharacter = String.valueOf((char) characterValue);
					...
					if (currentlyReading) {
						...
						countedLength++;
						// once we reach the target length, we put the entry into the dictionary,
						// increment the index, and reset currentString, countedLength, and
						// currentlyReading
					...
					// if we are not reading a significant part
						...
						// if we're on a delimiter, then we record the target length of the next
						// combination and reset currentString, and set currentlyReading to true
